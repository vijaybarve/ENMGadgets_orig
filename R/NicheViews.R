...
#' 
#' @import raster
#' @param OutputSufix - Suffix for the output graph files. (file names will be "sample12.jpg" where sample is suffix and component 1 and 2 are plotted in this plot)
#' @param RasterFileList - Component files in .asc format (Use the components generated by PCA function, you can use any .asc files if you need)
#' @param DataFileName - Species occurrences file - .csv file with "Species,Longitude,Latitude" fields with headers. 
#' @examples \dontrun{
#' NicheViews()
#' }
#' @export
NicheViews <- function(OutputSufix=NA,RasterFileList=NA,DataFileName=NA)
{
  ...
  cat("Stacks created\n")
  if(is.na(FileName)){ 
    FileName = file.choose("Select occurrence points : ")
  }
  bpt1 = rasterToPoints(st1)
  cat("Points generated\n")
  bpt1 = as.data.frame(bpt1)
  compname = names(bpt1)
  oMat = read.table(FileName, header=T, sep = ",")
  xypt = oMat[,2:3]
